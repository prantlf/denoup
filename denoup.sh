...
readonly INST_NAME=denoup
readonly LANG_NAME=Deno
readonly TOOL_NAME=deno
...
if [[ "$ARG" != "" ]] && ! [[ "$ARG" =~ ^[.[:digit:][:alpha:]]+$ ]] && [[ "$ARG" != "latest" ]] && [[ "$ARG" != "lts" ]]; then
...
detect_platform() {
    ...
    if [ -z "$PLATFORM" ]; then
        ...
        if ! [[ " x86_64 aarch64  " =~ [[:space:]]${ARCH}[[:space:]] ]]; then
        ...
        if ! [[ " darwin-x86_64 darwin-aarch64 linux-x86_64 linux-aarch64 windows-x86_64 " =~ [[:space:]]${PLATFORM}[[:space:]] ]]; then
    ...
    REPO_URL=${REPO_URL-https://github.com/denoland/deno}
    API_URL=${API_URL-https://api.github.com/repos/denoland/deno}
    PKG_NAME=${PKG_NAME-deno-$ARCH-$VENDOR-$OS$BUILD.zip}
...
do_update_installer() {
    ...
    if [[ "$LATEST_VER" != "$VERSION" ]]; then
        ...
        exec curl -f "$PROGRESS" "$INST_URL" | NO_INSTRUCTIONS=1 DO_UPGRADE=$DO_UPGRADE UPGRADE_ARG=$UPGRADE_ARG bash $TRACE ||
